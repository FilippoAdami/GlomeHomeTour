...
from dataset import GSInputDataset, GSSceneData
from density_control import DensityControlConfig
from model import Material2DGSModel
from trainer import Material2DGSTrainer, TrainerConfig
...
def resolve_scene_paths(
    ...
    if require_ply and (resolved_ply is None or not resolved_ply.is_file()):
...
def train_scene_from_ply_and_frames(
    ...
    random_init_points: int = 300_000,
) -> Dict[str, Any]:
    """Execute complete end-to-end 2DGS training from PLY surfels and keyframe package.

    random_init=True bypasses the depth-prior surfel cloud entirely: the model is
    seeded with a uniform random point cloud inside the camera trajectory's bounding
    box (vanilla-3DGS style), and depth/normal priors are not loaded, so the run
    isolates how the training loop (density control, rasterizer) performs on its own.
    """
    # 1. Resolve Paths
    ...
    # 3. Load & Downsample Surfel Cloud (or synthesize a random one)
    t0 = time.time()
    if random_init:
        with open(actual_gs_input / "transforms.json", "r", encoding="utf-8") as f:
            _tf = json.load(f)
        cam_pos = np.array([fr["transform_matrix"] for fr in _tf["frames"]], dtype=np.float32)[:, :3, 3]
        margin = 1.0  # meters, so the random cloud extends past the camera path to cover walls/ceiling
        bbox_min = cam_pos.min(axis=0) - margin
        bbox_max = cam_pos.max(axis=0) + margin
        print(f"[1/4] Random init (no depth prior): {random_init_points:,} points in bbox {bbox_min} -> {bbox_max}")
        surfel_cloud = SurfelCloud.from_random(bbox_min, bbox_max, num_points=random_init_points)
    else:
    ...
    # Radial depth ceiling from the trusted initial surfel cloud: caps how far
    # from the world origin a densified/split primitive may drift before being
    # pruned as a floater. Without this, density_control's depth-ceiling prune
    # is inert (max_depth_ceiling defaults to None) and nothing ever culls
    # splats that grow outside the actual room bounds.
    surfel_radii = np.linalg.norm(surfel_cloud.positions, axis=1)
    depth_ceiling_m = float(np.percentile(surfel_radii, 99.5)) * 1.2
    print(f"      Radial depth ceiling for floater pruning: {depth_ceiling_m:.2f}m")
    ...
    # 5. Initialize Dataset Loader & Select Optimal Keyframes
    ...
    print(f"      Total candidate frames: {len(dataset.frames_meta)}. Depth keyframes: {len(dataset.depth_keyframe_indices)}")

    # Select frames between K and 2K, or use every candidate frame when
    # use_all_frames requests maximum multi-view supervision density
    # (denser depth-prior coverage, harder for a floater to stay consistent
    # across more viewpoints) at the cost of proportionally longer stage
    # data loads.
    if use_all_frames:
        selected_frame_indices = list(range(len(dataset.frames_meta)))
        print(f"      Using all {len(selected_frame_indices)} candidate frames for 2DGS training (--use-all-frames).")
    else:
        selected_frame_indices = dataset.select_frames_between_k_and_2k()
        print(
            f"      Selected {len(selected_frame_indices)} frames for 2DGS training "
            f"({len(dataset.depth_keyframe_indices)} depth anchors + {len(selected_frame_indices) - len(dataset.depth_keyframe_indices)} intermediate)."
        )

    # Save selected frames to selected_images/ directory for user inspection
    selected_img_dir = out_path / "selected_images"
    selected_img_dir.mkdir(parents=True, exist_ok=True)
    import shutil
    for rank, idx in enumerate(selected_frame_indices):
        f_meta = dataset.frames_meta[idx]
        rel_path = f_meta["file_path"]
        src = actual_gs_input / rel_path
        if src.is_file():
            is_kf = "depth_anchor" if idx in dataset.depth_keyframe_indices else "intermediate"
            dst = selected_img_dir / f"frame_{rank:04d}_orig_{idx:04d}_{is_kf}_{Path(rel_path).name}"
            if not dst.exists():
                shutil.copy2(src, dst)

    # 6. Configure Multi-Scale Progressive Resolution Scheduling (4 Stages for 6,000 iters)
    base_h, base_w = dataset.base_h, dataset.base_w
    if multi_scale and iterations >= 3000:
        it_360 = min(1000, int(iterations * (1000 / 6000)))
        it_720 = min(3000, int(iterations * (3000 / 6000)))
        it_1080_a = min(5000, int(iterations * (5000 / 6000)))
        stages = [
            {
                "name": "Stage 1: Coarse Radiance & Aggressive Geometry Correction (360p)",
                "iters": (1, it_360),
                "res": (base_h // 3, base_w // 3),  # (640, 360)
            },
            {
                "name": "Stage 2: Macro Structural Growth & Splitting (720p)",
                "iters": (it_360 + 1, it_720),
                "res": (int(base_h * 2 / 3), int(base_w * 2 / 3)),  # (1280, 720)
            },
            {
                "name": "Stage 3: Native 1080p Detail & Topology Freeze",
                "iters": (it_720 + 1, it_1080_a),
                "res": (base_h, base_w),             # (1920, 1080)
            },
            {
                "name": "Stage 4: 1080p Specular Radiance & Photometric Polish",
                "iters": (it_1080_a + 1, iterations),
                "res": (base_h, base_w),             # (1920, 1080)
            },
        ]
    elif multi_scale and iterations >= 1000:
        ...
        stages = [
            {
                ...
                "iters": (1, it_360),
                "res": (base_h // 3, base_w // 3),
            },
            {
                "name": "Stage 2: Macro Structural Growth (720p)",
                "iters": (it_360 + 1, it_720),
                "res": (int(base_h * 2 / 3), int(base_w * 2 / 3)),
            },
            {
                "name": "Stage 3: Native 1080p Detail & Topology Freeze",
    ...
    densify_start = min(500, int(iterations * (500 / 6000))) if iterations >= 3000 else 50
    densify_stop = min(4500, int(iterations * (4500 / 6000))) if iterations >= 3000 else int(iterations * 0.7)
    ...
    trainer_cfg = TrainerConfig(
        ...
        lr_position_init=8.0e-4 if iterations >= 3000 else 1.6e-4,
        lr_position_max_steps=iterations,
        lr_opacity=0.005,
        gamma_normal=0.0 if random_init else 0.15,
        gamma_depth=0.0 if random_init else 0.20,
        max_depth_ceiling=depth_ceiling_m,
    ...
    initial_scene_data = dataset.load_scene_data(
        ...
        device=None,
        load_depth_normals=not random_init,
    ...
    def save_stage_snapshot(it_num: int, res: Tuple[int, int], stats: Optional[Dict[str, Any]] = None) -> None:
        ...
        from export_standard_ply import export_model_to_standard_3dgs_ply
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir",
        type=str,
        default="backend/scenes/bedroom_complete_2DGS_results",
        help="Output directory for 2DGS model and bundle.",
    )
    parser.add_argument("--iterations", type=int, default=6000, help="Total training iterations.")
    ...
    parser.add_argument("--use-all-frames", action="store_true", help="Skip K-2K frame subsampling; train on every candidate frame.")
    parser.add_argument("--random-init", action="store_true", help="Bypass the depth-prior surfel cloud: seed with a random point cloud and disable depth/normal priors (isolates the training loop).")
    parser.add_argument("--random-init-points", type=int, default=300_000, help="Number of points for --random-init.")
