...
class Camera(nn.Module):
    ...
    def enable_pose_refinement(self):
        ...
        self.pose_delta = nn.Parameter(torch.zeros(6, device="cuda"))

    def refresh_pose(self):
        """(Re)derive the view matrices from the base pose and the pose delta.

        Kept as plain attributes rather than properties because utils/render_utils.py
        assigns world_view_transform / full_proj_transform / camera_center directly
        when rendering custom trajectories.
        """
        wvt = self.base_world_view_transform
        if self.pose_delta is not None:
            from utils.pose_refine import se3_exp
            # These matrices are W2C transposed (row-vector convention), so the
            # delta is composed in W2C space and transposed back.
            wvt = (se3_exp(self.pose_delta) @ wvt.T).T
