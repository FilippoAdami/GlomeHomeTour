...
class TamingDensityController:
    ...
    def densify_and_prune(
        ...
        Args:
            ...
            grow: If False, skip clone/split entirely and only run the prune
                pass -- used to keep culling floaters after topology freeze
                (densify_stop_iter) without growing the primitive count.
        ...
        # Carry "is_original" provenance across the same reindexing as the
        # other tensors: kept originals inherit their old flag, every fresh
        # clone/split child (src_idx == -1) is marked non-original.
        new_is_original = torch.zeros(new_xyz.shape[0], dtype=torch.bool, device=device)
        valid_src = src_idx >= 0
        new_is_original[valid_src] = model.is_original[src_idx[valid_src]]
        model.is_original = new_is_original
