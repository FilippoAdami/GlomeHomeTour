...
from compressor import LightGaussianCompressor
from density_control import DensityControlConfig, TamingDensityController
from losses import ReconstructionLoss
from model import Material2DGSModel
from pbr_shader import DeferredCookTorranceShader
from rasterizer_interface import Base2DGSRasterizer, get_default_rasterizer
...
class TrainerConfig:
    ...
    lr_position_init: float = 8.0e-4          # High mobility in early stage to snap floaters onto surfaces
    lr_position_final: float = 1.6e-6
    lr_position_delay_mult: float = 0.01
    lr_position_max_steps: int = 6000
    lr_rotation: float = 1e-3
    lr_scaling: float = 5e-3
    lr_opacity: float = 5e-3             # 0.005 (preserves solid alpha >= 0.85)
    lr_albedo: float = 2.5e-3
    camera_convention: str = "opengl"  # ARCore/transforms.json cameras are -Z forward
    lambda_ssim: float = 0.2
    gamma_normal: float = 0.15           # Strong normal prior regularization
    gamma_depth: float = 0.20            # Strong depth prior penalty to suppress free-space floaters
    gamma_opacity_sparsity: float = 0.01  # L1 pull toward opacity=0; only reconstruction-loss
                                           # gradient keeps a splat opaque, so floaters with weak
                                           # support decay into the min_opacity_prune cutoff
    densify_start_iter: int = 500        # Start active growth at Stage 2
    densify_stop_iter: int = 4500        # Freeze topology (no more clone/split) after step 4500
    prune_stop_iter: Optional[int] = None  # Keep pruning (no growth) until here; defaults to total_iterations
...
class Material2DGSTrainer:
    ...
    def train_step(self, iteration: int) -> Dict[str, float]:
        ...
        # Opacity sparsity: nothing else in the loss penalizes a splat for
        # merely existing, so floaters that marginally help one view never
        # lose opacity on their own. This L1 pull toward 0 is opposed only
        # by the reconstruction-loss gradient -- but that gradient only
        # reaches a splat's opacity on iterations where it's actually
        # rendered, while the flat penalty applied to every splat regardless
        # of visibility this step. Restricting it to non-original (grown)
        # splats keeps the depth-prior-anchored initial cloud (walls, floor,
        # ceiling) completely unpunished -- only cloned/split splats, which
        # is where floaters actually originate, get pulled toward 0.
        non_original_mask = ~self.model.is_original
        if self.config.gamma_opacity_sparsity > 0.0 and bool(non_original_mask.any()):
            loss_opacity_sparsity = self.model.opacity[non_original_mask].mean()
            total_loss = total_loss + self.config.gamma_opacity_sparsity * loss_opacity_sparsity
            metrics["loss_opacity_sparsity"] = loss_opacity_sparsity
        ...
        prune_stop_iter = self.config.prune_stop_iter or self.config.total_iterations
        # Growth freezes at densify_stop_iter, but floaters born or reinforced
        # afterward (e.g. overfitting a single hard view in the native-res
        # polish stages) were never pruned again. Keep a prune-only pass
        # running past the freeze so the final export isn't stuck with them.
        should_prune_only = (
            not should_grow
            and iteration > self.config.densify_stop_iter
            and iteration <= prune_stop_iter
            and iteration % self.config.densify_interval == 0
        )
        if should_grow or should_prune_only:
        ...
        for key in ("loss_normal", "loss_depth", "loss_opacity_sparsity"):
            if key in metrics:
