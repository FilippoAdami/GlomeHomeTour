...
from ingestion.package_loader import load_package
...
def main():
    ...
    print("\n[2/4] Running Desktop Quality Gate (Blur, Baseline Redundancy, Exposure)...")
    t1 = time.time()
    gate = QualityGate(blur_threshold=args.blur_thresh, adaptive_blur=True)
    ...
    print(f"    - Rejected (redundancy):     {gate_res.summary['rejected_redundancy']}")
    ...
    if args.run_depth:
        ...
        if args.dynamic_keyframes:
            ...
            sel_res = selector.select_keyframes(candidate_pool, intr)
