...
Resuming is the default: each step records itself in ``pipeline_stats.json`` and
...
def print_stats_summary(workspace: Path, order: list[str]) -> None:
    """Print high-level pipeline_stats.json summary to stdout."""
    stats = read_pipeline_stats(workspace)
    lines = ["=" * 70, " PIPELINE STATS SUMMARY", "=" * 70, f"Workspace: {workspace}", ""]

    for name in order:
        entry = stats.get(name)
        if entry:
            lines.append(f"{name:<16} total_time: {entry.get('total_time', 0):>7.2f}s | "
                         f"frames: {entry.get('frames_processed', 0):>4} | "
                         f"time_per_frame: {entry.get('time_per_frame', 0):>8.4f}s")
        else:
            lines.append(f"{name:<16} (not run)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("source", nargs="?", default=DEFAULT_SOURCE,
                        help=f"Capture .zip or extracted directory (default: {DEFAULT_SOURCE})")
    parser.add_argument("--workspace", default=str(DEFAULT_WORKSPACE))
    parser.add_argument("--from-step", choices=STEP_NAMES)
    parser.add_argument("--to-step", choices=STEP_NAMES)
    ...
    if args.only_step:
        ...
        end = STEP_NAMES.index(args.to_step) + 1 if args.to_step else len(STEP_NAMES)
        selected = STEP_NAMES[start:end]
    ...
    for name, module_path in STEPS:
        ...
        if rc != 0:
            ...
            print_stats_summary(workspace, selected)
            return rc
